...
void render(std::vector<glm::vec3> VBO, const Uniform& uniforms, const int shader) {
  ...
  for (Fragment fragment : fragments) {
    if (shader == 0){
      point(fragmentShaderShip(fragment));
    } else if (shader == 1) {
      point(fragmentShaderPluto(fragment));
    } else if (shader == 2) {
      point(fragmentShaderEarth(fragment));
    } else if (shader == 3) {
...
glm::mat4 createModelMatrixNave() {
  ...
  glm::mat4 translation = glm::translate(glm::mat4(1), glm::vec3(xNave + cameraDirection.x, yNave + cameraDirection.y, zNave + cameraDirection.z));
  float spaceShipScale = 0.2f;
  glm::mat4 scale = glm::scale(glm::mat4(1), glm::vec3(spaceShipScale, spaceShipScale, spaceShipScale));
...
int main() {
  ...
  if (loadOBJ("assets/StarFighter.obj", verticesNave, texturesNave, normalsNave, facesNave)) {
    // For each face
  ...
  if (loadOBJ("assets/planet.obj", verticesPlaneta, texturesPlaneta, normalsPlaneta, facesPlaneta)) {
    // For each face
